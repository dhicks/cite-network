...
def get_dois_by_issn(issn, after = '2004', before = '2016'):
    ...
    query = (base_query + search_string + '&' +
                ...
                'apiKey=' + MY_API_KEY)
    # TODO: for some reason xmltodict doesn't like the xml that's being returned. 
    # try getting json back instead
    response_raw = requests.get(query, headers = {'X-ELS-APIKey': MY_API_KEY})
    response = xmltodict.parse(response_raw.text)
    total_papers = response['search-results']['opensearch:totalResults']
    return(total_papers)
...
sources = []
for paper in gen_0 + gen_n1 + gen_n2:
